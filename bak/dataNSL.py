...
class KddData(object):
    ...
    def __init__(self, batch_size, train_data,train_target, test_data,test_target):
        ...
        data_X, data_y = self.__encode_data(train_data, train_target)
        data_xtest, data_ytest = self.__encode_data(test_data, test_target)
        ...
        self.test_dataset = TensorDataset(
            torch.from_numpy(data_xtest.astype(np.float32)),
            torch.from_numpy(data_ytest.astype(np.int64))
        )
